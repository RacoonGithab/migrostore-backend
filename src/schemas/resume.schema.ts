...
export const createResumeSchema = z.object({
    title: z.string().regex(regexPatterns.TITLE_POSITION_SPECIALTY_NAME, { message: "The position name is required." }),
    ...
    phoneNumber: z.string().regex(regexPatterns.PHONE_NUMBER, "Invalid phone number format. (e.g., +380991234567)"),
    email: z.string().regex(regexPatterns.EMAIL, {message: "Invalid email format"}).nonempty("Email is required"),
    dateOfBirth: z.string().regex(regexPatterns.END_DATE, {message: "Date of birth must be in YYYY-MM-DD format."}),
    aboutMe: z.string().regex(regexPatterns.ABOUT_ME, "Invalid description format"),
    country: z.string().regex(regexPatterns.COUNTRY, {message: "Invalid country"}).optional(),
    qualification: z.string().regex(regexPatterns.TITLE_POSITION_SPECIALTY_NAME, {message: "Invalid title format"}).optional(),
